functions {
  ...
  // backward algorithm for calculating log p(y[n+1 : N] | z[n] == k)
  array[,] real backward() {
    int N = size(y);
    int K = size(pi);
    
    array[K] real acc;        // temporary variable
    array[N, K] real beta;    // log p(y[n+1 : N] | z[n] == k)
    
    for (j in 1:K)
      beta[N, j] = 1;
    for (nforward in 0:(N-2)) {
      int n = N - nforward;
	...
	for(i in 1:K) {
	  // j = previous (n-1)
	  // i = next (n)
	  //                backwards n + transition prob + local evidence at n
          accumulator[i] = beta[n, i] + log(theta[j, i]) + lognormal_mix_lpdf(y[n] | i, mu, sigma, y_max);
	}
        beta[n-1, j] = log_sum_exp(accumulator);
      }
    }
